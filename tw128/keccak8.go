...
const (
	...
	lanes      = stateBytes / 8
	rate       = 168
	// rhoBytes is the maximum number of sigma bytes a single duplex block can
	// absorb: the rate (168 bytes) less the byte that holds the 4-bit domain
	// suffix and the pad10*1 bits.
	rhoBytes = rate - 1
)
...
// mask64 returns a mask selecting the low n bytes of a 64-bit word, for n in
// 0..7.
