...
The candidate machinery rests on a single probabilistic engine, stated
here and proved in full in \Cref{app:lem-candidate-collisions}:
adaptively chosen distinct random oracle inputs whose values are still
unread when they are chosen behave, for collision and target-hitting
purposes, as fresh uniform draws.
...
unrevealed-value premise guarantees that no bit of that value has been
...
read by a single $\ROflat$ lookup at $\flatmap(R_i)$. Call an $\ROflat$
input a \emph{root tag input} if it decodes under $\KeyedTWOut$ to a root
tag output-point class---$\Rmsg^+$ (the final root message block, which
carries the root tag only on the no-leaf path) or $\Rtag$ (the aggregated
root tag); multi-chunk computations also reach $\Rmsg^+$ with zero
requested output before aggregation, so this predicate over-approximates
the root tag candidate set, which only loosens the count. Every execution
...
not a separate construction query.
...
\emph{Predictability.} The append predicate depends on the input string
alone through the exact decoder of \Cref{lem:bridge-decode}, which ignores
the requested output length, and by Output-Point Separation
(\Cref{cor:output-sep}) each input decodes to at most one output-point
class. The string itself is fixed in the state preceding the trigger: a
direct query's input is chosen by $\mathcal{B}$ before its lookup, and a
check's final root transcript is determined by the check's input and the
prior oracle answers---including any private challenger state sampled
before the candidate's unread value, which \Cref{lem:adaptive-candidate}
expressly allows.
...
positive number of its bits. The only positive-length construction
lookups at root tag inputs are the checks' root tag samplings, which are
themselves triggers; every other intermediate construction lookup either
lies at a stream output point---a different class and hence, by
\Cref{cor:output-sep}, a different address---or is the
zero-requested-output $\Rmsg^+$ lookup a multi-chunk computation makes
before aggregation, which the unrevealed-value premise of
\Cref{lem:adaptive-candidate} expressly permits, including when that
zero-length lookup lands at another check's future no-leaf final-root
address. Together with predictability this gives~(2).

\emph{Length.} Appends are triggered only by direct queries---at most
$\qRO(\mathcal{B})$ of them---or by the $m$ root tag samplings;
zero-length lookups never trigger an append. Hence the sequence has
length at most $\qRO(\mathcal{B}) + m$, giving~(3).
\end{proof}
...
\begin{proof}
The Sec and eSec notions of \cite{RS04} are the slice-parameterized
$\mathrm{Sec}[m]$ and $\mathrm{eSec}[m]$, and \cite[Proposition~6]{RS04}
shows that collision resistance implies both on each fixed slice.
Applying the implication on the slice $\mathcal{X}_{a,\mu}$ to the
two-way collision bound of \Cref{cor:coll-rs04}, which does not depend
on the slice, gives the stated bound for every $(a, \mu)$.
\end{proof}
