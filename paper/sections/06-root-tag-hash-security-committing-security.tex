...
(\Cref{lem:root-tag-hash}) turns a collision or fixed-target preimage among the
...
everywhere preimage (\Cref{thm:epre-ro})---are then short instantiations,
differing only in which case of the bound they invoke and in whether a leaf tag
collision term is needed when colliding inputs may use different ciphertext
bodies. CMTDs-4
...
\begin{proof}
The committing and hash (collision and everywhere preimage) games are keyless:
all keys are adversary-supplied, so a direct
...
(\Cref{thm:coll}), second-preimage-resistant
...
\begin{proof}
Collision resistance is \Cref{thm:coll}, second-preimage resistance follows
from collision resistance by \Cref{cor:second-preimage-rs04}, and everywhere
preimage resistance is \Cref{thm:epre}; these guarantees quantify
