...
\]
Substituting into
\Cref{cor:ae,thm:mu-ind,thm:int-rup,thm:coll,cor:second-preimage-rs04,cor:cmtds4,thm:epre}
...
$\HTW$---$s$-way collision resistance for every $s \geq 2$, standard and
everywhere second-preimage resistance inherited from collision resistance, and
everywhere preimage resistance
(\Cref{thm:coll,cor:second-preimage-rs04,thm:epre})---and the \cite{BH22}
...
\textsf{INT-RUP} & $\Lift_c$ and birthday terms & $\leq 2^{-128}$ \\
...
At $\Ntot \leq 2^{63}$, the single-user, hash, and CMTDs-4 bounds have
flat-sponge and, where present, root or leaf birthday terms on the same
$2^{-131}$ scale. Thus the flat sponge loss is not always the largest
summand: at the extreme $s=2$ root-collision cap
$\Nprim=\Ntot=2^{63}$, $\binom{\Nprim+2}{2}/2^{256}$ is slightly larger
than $\Lift_c(\Ntot)$. The multi-user bound is the exception: its
hybrid-induced
...
\item \emph{$\RootPre_\tau(Q) = (Q + 1) / 2^\tau$.} Tight at constants: a
  ...
  preimage (ePre) bound, where the single fixed target yields a linear
  term.
...
\item \emph{Known-key leaf collision $\LeafColl_\tau(Q)$.} The same
  ...
  queries can target leaf tag points; the collision proof uses
  $Q=\Nprim+\Nleaf$.
