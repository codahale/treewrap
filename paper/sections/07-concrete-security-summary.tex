...
everywhere second-preimage resistance inherited from collision resistance,
standard preimage resistance on every fixed input slice, and everywhere
preimage resistance
...
linear root-preimage, key-guessing, or verification term with no factor $D$
is at most a $2^{-193}$-scale term.
...
$\HTW$ preimage (Pre) & $\Lift_c$, $\LeafColl$, $\RootPre$, slice term & $\leq 2^{-128}$ \\
