...
analysis into AEAD bounds and root-tag hash, committing, and context
discoverability bounds. This section instantiates those bounds at the
...
$\mathsf{CDY}^\ast$ context discoverability (\Cref{thm:cdy-full}).
\end{corollary}
...
$\mathsf{CDY}^\ast$ context discoverability bounds are dominated by the flat
sponge loss, except when a root or leaf birthday term reaches the same
$2^{-131}$ scale. The multi-user bound is the exception: its hybrid-induced
...
\item \emph{$\RootPre_\tau(Q) = (Q + 1) / 2^\tau$.} Tight at constants: a
  ...
  preimage (ePre), always second-preimage (aSec), and context discoverability
  ($\mathsf{CDY}^\ast$) bounds, where the single fixed target yields a
  linear term.
