...
needs
one construction transcript per targeted user, charging $N \geq D'$ for
$D'$ targeted users, so under the joint cap $\Ntot \leq 2^{63}$ the
corner forces $N = 0$; the attack-feasible maximum of the term is
$\approx 2^{-132}$, at $D' = \Nprim = 2^{62}$, and the remaining factor
is hybrid slack from charging created-but-idle users. Expressed in bytes, $\Ntot \leq 2^{63}$ corresponds to
...
\item \emph{$\KeyGuess_k(Q) = Q / 2^k$.} Tight at constants: a direct
  ...
  leading constant in the theorem's free-parameter setting: the
  adversary first obtains one encryption transcript per targeted
  user---construction work charged to $N$, which does not enter the
  term---then tests each direct guess against the collected
  transcripts; each guess matches some targeted key with probability
  $D / 2^k$, so $\Nprim$ guesses recover a user key---and
  distinguish---with probability $\approx D\Nprim / 2^k$, matching the
  bound. Under the joint cap the extreme corner is unreachable, as
  noted in the concrete-cap paragraph above.
...
\item \emph{Flat sponge lift.} The term
  ...
  improvable simulator bound, and is not specific to \TW{}.
\end{itemize}
