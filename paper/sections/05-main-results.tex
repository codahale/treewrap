...
\TW{} delivers three headline guarantees in the lifted
public-permutation model of \Cref{sec:games-pp}: all-in-one AE
security \cite{RS06}, its multi-user indistinguishability lift
\cite{BT16}, and $s$-way CMTDs-4 committing security
\cite{BH22}. For the AE-style notions the ideal side exposes the
BDPV simulator; committing security remains a real public-permutation
success probability. \Cref{thm:ae,thm:mu-ind,thm:cmtds4} state these
bounds; the loss terms $\Lift_c$, $\KeyGuess_k$, and $\RootColl_\tau$
...
Three closed-form expressions recur across the headline bounds of
\Cref{sec:headline}---$\Lift_c$ in all three, $\KeyGuess_k$ and
$\RootColl_\tau$ in some. We collect them here for reference.

\paragraph{Flat-sponge loss.}
The flat-sponge differentiating loss uses the transformation-form
expression of \cite[Eq.~(4)]{BDPV08},
\[
  \Lift_c(\Ntot) = \Ntot(\Ntot + 1) / 2^{c + 1}.
\]
For a random permutation, \cite[Eq.~(6)]{BDPV08} gives the bound
$\bigl((1 - 2^{-r})\Ntot^2 + (1 + 2^{-r})\Ntot\bigr)/2^{c+1}$, which
never exceeds the transformation-form expression: the $2^{-r}$
reduction in the quadratic coefficient dominates the $2^{-r}$ increase
in the linear coefficient for $\Ntot \geq 1$. The transformation-form
expression therefore remains a valid (conservative) upper bound in the
random-permutation setting. The theorem carries the side condition
$\Ntot < 2^c$; for
...
The headline theorems of \Cref{sec:headline} feed on three
random-oracle-model envelopes whose proofs occupy
...
\]
Substituting into \Cref{thm:ae,thm:mu-ind,thm:cmtds4} and using the
random-oracle envelopes of \Cref{sec:thms-pp} gives the following
closed-form bounds.
...
\TW{} attains a $128$-bit security target across all three notions of
...
hybrid term in the mu-ind bound. The headline work cap
...
three. We comment on dominance and tightness.

\paragraph{Dominance.}
In the single-user AE bound, the birthday-on-capacity loss
$\Lift_c(\Ntot) = \Ntot(\Ntot + 1) / 2^{257}$ dominates throughout
the non-trivial range $\Ntot < 2^{c}$. The key-guessing and
...
1)}$ in the denominator for $s \geq 3$.
...
$\approx 4 \Lift_c$ and is the binding constraint; the user-key
birthday $D(D - 1) / 2^{257}$ and the leaf-tag collision each
contribute roughly $\Lift_c$. The $2^{-128}$ figure of
\Cref{sec:concrete-claim} is the sum of these terms at the
threshold.
...
\item \emph{Flat-sponge lift.} The term
  $\Lift_c(\Ntot) = \Ntot(\Ntot + 1) / 2^{c + 1}$ is the
  transformation-form flat-sponge loss of \cite[Eq.~(4)]{BDPV08}, used
  as a random-permutation upper bound via \Cref{sec:loss-terms}. The
