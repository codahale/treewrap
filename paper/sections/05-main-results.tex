...
before $\badkey^{(d)}$; any extra lazy-table entries
...
\]
For a random permutation, \cite[Eq.~(6)]{BDPV08} gives a strictly
smaller bound: its $2^{-r}$ terms lower the leading coefficient from
$1$ to $1 - 2^{-r}$. The transformation-form expression therefore
remains a valid (conservative) upper bound in the random-permutation
setting. The theorem carries the side condition
$\Ntot < 2^c$; for
...
input node is rooted, unsaturated, paddable, and a new edge in the
simulator's internal graph; inverse queries and forward queries
violating any of those conditions return $\ROflat$-free outputs.
