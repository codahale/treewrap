...
\begin{proof}
The CMTDs-4 game is the special case of \Cref{thm:tag-cmtds4} in which
all $s$ openings share a single ciphertext body. With one common body no
leaf tag collision can merge two openings, so the $\LeafColl$ term is
absent and only the root-collision bound remains. Concretely, for
deterministic $\Dec$ every CMTDs-4 winner is also a CMTDs-3 winner,
yielding $\Advcmtds{4}_\TW(\mathcal{A}) \leq \Advcmtds{3}_\TW(\mathcal{A})$
by \Cref{app:committing-transfer}, and the public-permutation CMTDs-3
bound \Cref{cor:cmtds3-pp} then gives the claim.
\end{proof}
