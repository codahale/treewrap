...
required; only the mu-ind user cap $D$ remains an independent hypothesis,
since created-but-idle users need not contribute to $N$. At the headline
...
$\Nprim, \Qv, \Nleaf \leq \Ntot$ of \Cref{sec:resources}. The committing
...
$s \geq 3$; the known-key $\LeafColl_{256}(\Nprim+\Nleaf)$ is below
$2^{-129}$; and the root-preimage term $(\Nprim+1)/2^{256}$ is at most
