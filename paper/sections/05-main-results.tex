...
\item \emph{Second-preimage resistance.} For every second-preimage
  ...
  particular $\Nprim \leq 2^{63}$), $\Advspr_\TW(\mathcal{A}) \leq
  2^{-128}$. The bound coincides termwise with the discoverability bound:
  the flat-sponge term is one $2^{-131}$-sized birthday term, up to
  lower-order summands, and the realized-target root-preimage term
  satisfies $(\Nprim + 1)/2^{256} \leq 2 \cdot 2^{-193}$, so the
