...
(\Cref{sec:commit-defs})---$s$-way CMTDs-4 committing security \cite{BH22}
and context discoverability \cite{MLGR23}---which follow from the same
root tag and are stated co-equally. We also state tag-projected
strengthenings showing that the root tag alone is the compact commitment
target. For
...
\begin{proof}
By the Lift Application corollary (\Cref{cor:lift-application}) for the
tag-CMTDs-4 game, the derived random-oracle adversary $\mathcal{B}$ has
...
tag-CMTDs-4 game, but the sharper no-$\LeafColl$ bound is not obtained by
...
\]
\cite[Lemmas~4 and~5]{BDPV08} give exact product-form upper bounds on
the differentiating advantage: the random-transformation bound is
$f_T(\Ntot) = 1 - \prod_{i=1}^{\Ntot}(1 - i/2^c)$, and the factor of
the random-permutation bound $f_P(\Ntot)$ corresponding to the
...
CMTDs-4 bound, the tag-CMTDs-4 bound, and the CDY bound---whose proofs occupy
...
\TW{} attains a $128$-bit security target across the notions of
\Cref{sec:concrete}. This is an instantiation of the closed forms above
...
Tag-CMTDs-4 & $\Lift_c(\Ntot)$ & $\leq 2^{-128}$ \\
CMTDs-4 & $\Lift_c(\Ntot)$ & $\leq 2^{-128}$ \\
Context discoverability & $\Lift_c(\Ntot)$ & $\leq 2^{-128}$ \\
\bottomrule
\end{tabular}
\end{table}
