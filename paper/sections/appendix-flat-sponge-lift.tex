...
$\Lift_c(\Ntot)$, where $\Ntot = N + \Nprim$ bounds the converted
\cite{BDPV08} primitive-query cost: construction-internal primitive calls
induced by construction queries and deterministic challenger computations,
after common-prefix deduplication, together with direct primitive queries.
...
queries. In the \cite{BDPV08} single-stage replacement, convert the
combined construction-and-primitive transcript to the primitive-query
sequence of \cite[Lemma~3]{BDPV08}, omitting duplicate primitive queries
induced by common prefixes. This converted sequence has \cite{BDPV08}
query cost at most
\[
  \Ntot = N + \Nprim .
\]
...
cost of the converted primitive-query sequence by $N+\Nprim=\Ntot$. If
