...
\Cref{cor:committing-combined}. The machinery applies uniformly to the
...
CMTDs-4, the $\HTW$ collision, second-preimage, and preimage games,
$\mathsf{CDY}^\ast[\mathsf{ts}]$, and CMTs-$\ell$ for
$\ell \in \{1,3,4\}$. In $\mathsf{CDY}^\ast[\mathsf{ts}]$ the selector
encryption is construction-internal, counted in $N$ like a challenger check.
\end{definition}
...
\begin{tabular}{@{}l p{0.62\linewidth}@{}}
  ...
  $\mathsf{CDY}^\ast[\mathsf{ts}]$ & fixed target/selector data and final output \\
  \bottomrule
\end{tabular}
\end{center}
...
final committing- or discoverability-game output) unchanged, and answers
...
$\Ver_K^\RO$, or a committing or discoverability challenger are forwarded to the
...
This is the only form used in \Cref{sec:headline}: every public-permutation
bound is obtained by adding $\Lift_c(\Ntot)$ and substituting
$\qRO \leq \Nprim$ in the corresponding random oracle theorem.
