...
by the tag hash bounds of \Cref{sec:root-tag-hash-security}, the committing
consequences of \Cref{cor:committing-combined}, and the baseline AEAD bounds of
\Cref{sec:aead-security}. The machinery applies uniformly to the games named
once here.
...
The \emph{single-stage \TW{} games} are the public-permutation games of
\Cref{sec:root-tag-games,sec:aead-games,sec:mu-setting} in which one
adversary interacts with the challenger in a single stage, so that a
\cite{BDPV08} replacement applies to an entire world's public-permutation
transcript at once, including deterministic challenger checks after the
adversary's final output. They are
the $\HTW$ tag hash games (collision and preimage games, standard and
everywhere), CMTDs-4, CMTs-$\ell$ for $\ell \in \{1,3,4\}$,
\textsf{INT-RUP}, all-in-one AE, and mu-ind. The win-event games among these
have one world; the real-vs-ideal games---all-in-one AE and mu-ind---have two,
and the lift below treats their real world by the full replacement and their
ideal world by the simulator--permutation exchange of
\Cref{lem:sim-perm-proximity}.
\end{definition}
...
\begin{tabular}{@{}l p{0.62\linewidth}@{}}
  ...
  \midrule
  $\HTW$ tag hash games & final structured hash game output \\
...
The simulator is a proof device: each public-permutation game of
\Cref{sec:root-tag-games,sec:aead-games,sec:mu-setting} states its advantage
over the real permutation, and the reduction to the corresponding random oracle model
...
of the tag hash and committing games.
...
than $2^c$, the simulator answers every query of the trace
unsaturated: its internal set $\mathcal{R} \cup \mathcal{O}$ contains
the initially rooted supernode $0^c$ and gains at most one element per
answered query, so when any query of the trace is answered the set has
fewer than $2^c$ elements and cannot equal the $2^c$-element capacity
space. The simulator's sponge-consistency invariant
(\cite[Lemma~2]{BDPV08}, stated for the transformation simulator and
extended to the permutation simulator in \cite[\S4.3]{BDPV08})
therefore makes the rate part returned at that node agree with the
corresponding lazy value $\ROflat(\flatmap(X))$. Thus the simulator-side operational construction
...
committing games, and the tag hash games' challenger encryptions---the
...
unchanged in the output-checked $\HTW$ tag hash and committing games. Then the
...
lazy $\ROflat$ table. The construction interface, tag hash challenger, or
...
\Cref{sec:root-tag-hash-security,sec:aead-security} are instantiated.
...
into a single template, stated once and then instantiated by the tag hash
bounds of \Cref{sec:root-tag-hash-security}, the committing consequences of
\Cref{cor:committing-combined}, and the baseline AEAD bounds of
\Cref{sec:aead-security}. Every
...
standard preimage game, the source encryption computing $T^*$ as well as the
...
$(\pi,\pi^{-1})$. The real-vs-ideal games of
