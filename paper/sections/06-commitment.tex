...
root tag inputs reached by the hash and committing games into a
...
second-preimage among those candidates into the matching adaptive bound of
\Cref{app:lemmas}, controlling the chance that distinct inputs land on the
same $\tauroot$-bit root tag projection. The
hash random oracle theorems---collision (\Cref{thm:coll-ro}), always
second-preimage (\Cref{thm:asec-ro}), and everywhere preimage
(\Cref{thm:epre-ro})---are then short instantiations, differing only in
which case of the bound they invoke and in whether a leaf tag collision
term is needed when colliding inputs may use different ciphertext bodies.
CMTDs-4 (\Cref{thm:cmtds4-ro}) and the remaining committing notions
reuse the collision case. The context-discoverability proof of
\Cref{thm:cdy-full} uses the same fixed-target root-preimage case, plus
hidden-component guessing.
