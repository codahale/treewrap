...
leaf transcripts that are distinct from the honest leaf transcript sharing
their leaf index, at most $\nleaf(\mathcal{B})$ in all. Each such check
leaf is generated construction-side and is appended immediately before its
leaf-tag oracle answer is sampled, exactly as for the root-tag candidates
above, so the candidates satisfy the predictability and unrevealed-value
premises of Revealed-Target Leaf Preimage; the candidate set is then
precisely the $\badleaf$ event. By Revealed-Target Leaf Preimage
