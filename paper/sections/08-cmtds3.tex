...
target set; the candidate leaf transcripts are $\mathcal{B}$'s direct
queried leaf transcripts together with the decryption check's leaf
transcripts, at most $\qRO(\mathcal{B}) + \nleaf(\mathcal{B})$ in all,
each distinct from the honest leaf transcript sharing its leaf index. By
Revealed-Target Leaf Preimage (\Cref{cor:revealed-target-leaf-preimage}),
\[
  \Pr[\badleaf]
  \;\leq\;
  \frac{\qRO(\mathcal{B}) + \nleaf(\mathcal{B})}{2^{\tauleaf}}.
...
absence of $\badleaf$ supplies the no-leaf-tag-collision side condition of
\Cref{lem:transcript-inj}, so $R_1 = R_0$ would recover the same absorbed
ciphertext $C_1 = C_0$, a contradiction. In either sub-case, by
injectivity of $\flatmap$ (\Cref{lem:bridge-decode}) hence
$\flatmap(R_1) \neq X_0$. The winning candidate therefore lies at an
address distinct from the target address $X_0$. As in \Cref{thm:cdy-ro}, if $\flatmap(R_1)$ was already touched by
...
equals the revealed target $T$. Applying \Cref{lem:revealed-target-preimage}
with $Q = \qRO(\mathcal{B})$, $\tau = \tauroot$, distinguished address
$X_0$, and revealed target $T$ bounds that event by
