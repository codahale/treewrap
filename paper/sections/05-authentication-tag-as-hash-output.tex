...
CMTDs-4, \Cref{cor:cmtds4}
  & Terms: $\Lift_c$, $\RootColl$. Ingredients: common ciphertext body;
    opening-triple separation. \\
...
\label{thm:epre-ro}
For every ePre adversary $\mathcal{B}$ in the \TW{}
...
\begin{proof}
Fix any query-independent target tag $T^* \in \bits^{\tauroot}$; the
bound below is uniform in $T^*$, so it bounds the worst case over
targets. Run the random oracle preimage game for $T^*$, which is
independent of $\ROflat$. After rejecting unless the
...
keyed-by-the-permutation convention of \Cref{sec:root-tag-defs} is the
...
\label{thm:epre}
For every ePre adversary $\mathcal{A}$ against $\HTW$ with the
resources of \Cref{sec:resources},
\[
...
\begin{proof}
By the Lift Application corollary (\Cref{cor:lift-application}) for the
everywhere-preimage game---whose target tag is fixed before the
primitive
...
by collision and second-preimage resistance. Standard preimage resistance
bounds the probability that an adversary with the stated query
resources finds an opening for the tag of a uniformly sampled
fixed-length opening; everywhere preimage
...
$\qRO(\mathcal{B}) \leq \Nprim$, and the random oracle bound of
\Cref{thm:cmtds4-ro} is nondecreasing in $\qRO(\mathcal{B})$, so the
monotone form of \Cref{cor:lift-application} gives the stated bound. The
challenger's $s$ decryption checks are construction computations whose
duplex calls are counted in $N$ and enter through $\Lift_c(\Ntot)$; their
$s$ final-root lookups are the candidate-sequence slack of
$\RootColl_{\tauroot}$ (\Cref{sec:loss-terms}).
\end{proof}
