...
games, encryption queries are nonce-respecting and the verification oracle
returns only acceptance or rejection. Thus the AE claims model deployments that
release plaintext only after successful root tag verification. They do not
claim nonce-misuse resistance, security under release of unverified plaintext,
or \textsf{INT-RUP}/plaintext-awareness security. The hash and committing games
are adversarial-initialization games over the submitted inputs $(K,U,A,M)$ or
ciphertext openings and impose no nonce uniqueness condition. Across all games,
the stated bounds are birthday-type, single-stage, and leakage-free:
beyond-birthday and side-channel security are outside the model.
