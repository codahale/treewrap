...
$\AdvPreSlice{a}{\mu}_{\RO}(\mathcal{B})$, and
  ...
  \begin{tabular}{@{}p{0.18\linewidth}p{0.42\linewidth}p{0.32\linewidth}@{}}
    ...
    $\Nprim$
      ...
      & Key-guess, root-collision, root-preimage terms; direct-query part of
        the lift. \\
    $\Ntot = N + \Nprim$
      & Total public-primitive query cost seen by the flat sponge
        replacement.
...
\item[$N$:] the construction interface cost, measured as the number of
  ...
  deterministic encryptions on the submitted inputs; for the standard
  preimage game it counts both challenger encryptions: the source
  encryption computing $T^* = \HTW(X^*)$ and the one deterministic
  encryption on the submitted input; for the everywhere-preimage game,
  the one deterministic encryption on the submitted input; for
  ...
  ciphertexts. For the Sec and eSec games of
  \Cref{cor:second-preimage-rs04}, $N$ and $\Nprim$ are those of the
  induced two-way collision adversary of \cite[Proposition~6]{RS04},
  whose two deterministic encryptions are counted exactly as in the
  $s = 2$ collision game.
