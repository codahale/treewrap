...
$\mathsf{CDY}^\ast$ form, in which the adversary supplies the entire
context, dominates the per-component restrictions $\mathsf{CDY}^\ast_k$,
$\mathsf{CDY}^\ast_n$, $\mathsf{CDY}^\ast_a$ of \cite[Fig.~5]{MLGR23}, in
which the selector additionally pins two of the three context components
and the adversary supplies only the remaining one. \cite{MLGR23} show that schemes which
...
The \TW{} instantiation below is the tag analogue: the target is a single
root tag $T^*$ fixed in advance, and the adversary wins by finding a
context and a ciphertext body that decrypt under $T^*$, with $(K,N,A)$ and
the body free. It is the preimage facet of the same compact commitment whose
collision facet is T-CMTDs-$4$: together they make \TW{}'s root tag
