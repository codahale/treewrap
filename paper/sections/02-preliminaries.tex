...
To express compact commitment, we also use a projection variant of the
D-notions. Let $\gamma : \mathcal{C} \to \mathcal{D}$ be a public
ciphertext projection. The $s$-way $\gamma$-CMTDs-$\ell$ game is the
same as CMTDs-$\ell$ except that the adversary outputs ciphertexts
...
pairwise-$\WiC_\ell$ checks as above. Thus ordinary CMTDs-$\ell$ is the
special case in which $\gamma$ is the identity projection, and a
nontrivial projection asks whether a shorter string is already a
binding commitment. For \TW{}, whose ciphertexts are written
$C \concat T$, the projection of interest is
\[
  \TagProj(C \concat T) = T .
\]
We call the resulting game tag-CMTDs-$\ell$. In tag-CMTDs-$\ell$, the
common value $T$ is the compact commitment string, while the ciphertext
bodies are opening data. This matches the compactly committing AEAD
intuition of \cite{GLR17}, but is stated as a projection refinement of
the \cite{BH22} CMTD games.
...
$C$ decrypts. The \TW{} instantiation below is already
tag-projected CDY, since it fixes only the root tag $T^*$ and leaves
the ciphertext body free.
