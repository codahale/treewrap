...
\label{def:htw}
For every \TW{} encryption input $X = (K,N,A,M)$ in the scheme domain,
define the root tag wrapper by
\[
  \HTW(K,N,A,M) = T
  \quad\text{where}\quad
  \Enc_K(N,A,M) = C \concat T.
\]
We also write $\HTW(X)$ for $\HTW(K,N,A,M)$. Equivalently, \HTW{} runs
...
security notions of \cite{RS04}, over the \TW{} input domain:
\begin{description}
...
valid target ciphertext together with the context components named by a
target specifier $\mathsf{ts} \subseteq \{k, n, a\}$, and the adversary,
given the revealed components, supplies the remaining components of a
...
query-independent selector $S$ whose output is supported on valid target
ciphertexts; equivalently, an adversary's restricted advantage is no
larger than its success probability against any fixed valid selector.
