...
\subsection{AEAD Syntax and Verification Interfaces}
\label{sec:ae-defs}
...
instantiated for \TW{} in \Cref{sec:aead-games}.

\subsection{Root Tag Hash Security}
\label{sec:root-tag-defs}
...
\begin{description}
  ...
  \item[Second-preimage resistance (Sec/eSec).] The standard and
    ...
    each fixed input slice $\mathcal{X}_{a,\mu}$, are reduced explicitly
    to two-way collision resistance in \Cref{cor:second-preimage-rs04}.
...
\TW{} in \Cref{sec:root-tag-games}. The Pre bound is proved directly in
\Cref{thm:pre}, while the second-preimage bounds are inherited from Coll
...
\subsection{AEAD Committing Notions}
...
$s$-way generalization because the root tag hash bounds of
\Cref{sec:root-tag-hash-security} are later instantiated in this AEAD
vocabulary.
...
$\Enc_{K_i}(U_i, A_i, M_i)$ are equal. Advantages
...
These definitions are instantiated for \TW{} in \Cref{sec:root-tag-games},
with the public permutation lift supplied by \Cref{app:flat-sponge-lift}.
...
\subsection{Integrity under Release of Unverified Plaintext}
\label{sec:rup-defs}
...
also accesses the public primitive interface $(\pi,\pi^{-1})$, exposed
identically in both worlds. This follows the same-oracle convention of
\cite{BT16}, who work in the ideal-cipher model and give both worlds
the same ideal-cipher oracles; here the shared primitive is the random
permutation. The adversary
...
\Cref{sec:mu-setting}, together with the public primitive interface
$(\pi,\pi^{-1})$, which does not depend on the challenge bit. We write
the user index as $d$ and the
