...
that would force each nonce value $N$ to appear under at most one user.
...
preimage resistance---captures the binding and preimage properties expected
of a compact commitment. The AEAD committing and discoverability notions are
then proved from the same tag analysis in \Cref{sec:main-results,sec:cmtds3}.
...
is a hash family, and we ask that it satisfy the hash-function security
notions of \cite{RS04}, over the \TW{} input domain:
\begin{description}
...
\end{description}
Their advantages $\AdvColl_\TW$, $\AdvaSec_\TW$, $\AdvePre_\TW$ are the
respective success probabilities, instantiated for \TW{} in
\Cref{sec:games-pp}. Coll is the binding property and ePre the preimage
property of the tag taken as a compact commitment to $(K, N, A, M)$---the
two properties a cryptographic hash commitment provides. Of the \cite{RS04}
...
general hash collision theorem (\Cref{sec:headline,sec:cmtds3}).
...
ciphertext, \emph{context discoverability security} \cite{MLGR23} forbids an
...
$\mathsf{CDY}^\ast[\mathsf{ts}]$ requires success against every
query-independent selector $S$; equivalently, an adversary's restricted
advantage is no larger than its success probability against any fixed
selector. Its three per-component variants $\mathsf{CDY}^\ast_k$,
...
nontrivial variant the selector fixes the \emph{entire} target ciphertext;
the full-reveal case $\mathsf{ts} = \{k,n,a\}$ hides no context component
and is excluded from \Cref{thm:cdy-full}.
...
context discovery attacks; the preimage character of the notion is what the
root tag preimage analysis addresses.

The preimage resistance of \HTW{} (ePre, above) is the tag-level preimage
notion that the \cite{MLGR23} context discoverability notion is the
ciphertext-level counterpart of; the two are distinct, and \TW{} is shown
to satisfy both (\Cref{thm:epre,thm:cdy-full}).
