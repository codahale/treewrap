...
support, nonce-misuse resistance, and full-input key commitment (\textsf{CMT-4}).
The comparison is structural; the concrete forms of \TW{}'s own privacy, AE, and
committing bounds, and the regimes in which each term dominates, are treated in
\Cref{sec:bound-discussion}. The \textsf{CMT-4} column records full-input key
...
listed schemes do not provide it.
...
resistance in the sense of \cite{RS06}; \textsf{CMT-4} is full-input
key-commitment in the sense of \cite{BH22}. A dash means the entry is inherited
from the base scheme. \TW{}'s concrete bounds appear in
\Cref{sec:bound-discussion}.}
\label{tab:scheme-compare}
\begin{tabular}{@{}llccccc@{}}
\toprule
Scheme & Primitive & Par. & K/N/T & AD & Misuse & \textsf{CMT-4} \\
\midrule
\TW{}                        & \Keccak[1600,12]   & yes & $32/32/32$ & yes & no  & yes \\
...
\TW{} instead pays for a larger permutation, \Keccak[1600, 12], and a $256$-bit
...
Keyak \cite{BDPVV16} is the closest prior duplex AEAD in spirit, since its
Motorist mode is already parallelizable. As discussed in \Cref{sec:related}, the
difference is that Keyak fixes its degree of parallelism as a mode parameter
that the encryptor and decryptor must share, whereas in \TW{} the number of
leaves evaluated at once is an implementation choice invisible to the ciphertext.
...
property of the mode, established without leaving the single-permutation duplex
setting.
...
from a single root-tag collision bound (\Cref{sec:cmtds3}). This places \TW{}
...
parallelism at constant memory (\Cref{sec:related,sec:impl}). Both demonstrate
...
$\Lift_c(\Ntot) = \Ntot(\Ntot+1)/2^{c+1}$ is the transformation-form
indifferentiability bound of \cite{BDPV08}, used here as a generic lift from the
