...
resistance in the \cite{MLGR23} sense: \TW{} provides it within the mode
...
leaf tag collision term $\Nleaf(\Nleaf-1)/2^{\tauleaf+1}$ is tight for INT-CTXT,
the notion it is proved against---a nonce-reusing adversary forges at that rate
by colliding two leaf tags---but the headline AE and multi-user bounds carry it
only through the INT-CTXT envelope, and the attack needs the nonce reuse their
nonce-respecting games forbid; for those bounds the term is inherited slack
rather than a matched one.
