...
The tag-level guarantees of this section share one spine. The Root tag
...
(\Cref{thm:coll-ro}), preimage (\Cref{thm:pre-ro}), and
always second-preimage (\Cref{thm:asec-ro})---are then short
instantiations, differing only in which facet they invoke and in whether
a leaf tag collision term is needed when colliding inputs may use
different ciphertext bodies. \Cref{app:flat-sponge-lift} transfers each
instance to the public permutation setting, \Cref{sec:headline} records
the concrete bounds, and \Cref{cor:committing-combined} forwards the
remaining \cite{BH22} committing notions to the same root-collision
bound.
...
$\tauroot$-projection are an $s$-way collision; the collision facet with
$L = \qRO + s$ gives
$\binom{\qRO + s}{s} \cdot 2^{-\tauroot(s-1)} = \RootColl_{\tauroot}(\qRO, s)$.
\emph{(2)} The candidate hitting the fixed target is a preimage; the
preimage facet with $L = \qRO + 1$ gives
...
with target $T^*$) gives
$\AdvPre_\RO(\mathcal{B}) \leq \RootPre_{\tauroot}(\qRO(\mathcal{B}))$.
\end{proof}
