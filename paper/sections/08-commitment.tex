...
headline CDY statement (\Cref{thm:cdy}).
Third, the T-CMTDs-4 bound (\Cref{thm:tag-cmtds4-ro}) keeps
...
public-permutation form is the headline \Cref{thm:tag-cmtds4}.
...
For every T-CMTDs-4 adversary $\mathcal{B}$ in the \TW{} random-oracle
model, let $\badleaf^\star$ be the event that two distinct final leaf
transcripts generated by the T-CMTDs-4 challenger decryption checks
...
\begin{proof}
Build a leaf tag candidate sequence for the T-CMTDs-4 game. During
...
\begin{theorem}[Random-Oracle T-CMTDs-4]
...
\begin{proof}
Run the random-oracle T-CMTDs-4 game. By
...
$(K_i,N_i,A_i,M_i)=(K_j,N_j,A_j,M_j)$, contradicting the T-CMTDs-4
