...
to the same root-collision bound. Second, the random-oracle T-CDY
...
headline T-CDY statement (\Cref{thm:cdy}).
...
\begin{theorem}[Random-Oracle T-CDY]
\label{thm:cdy-ro}
For every query-independent target tag $T^* \in \bits^{\tauroot}$ and
every T-CDY adversary $\mathcal{B}$ in the \TW{}
...
\begin{proof}
Run the random-oracle T-CDY game (\Cref{sec:ro-model}) for the
...
\Cref{lem:adaptive-candidate} with $Q = \qRO(\mathcal{B})$,
