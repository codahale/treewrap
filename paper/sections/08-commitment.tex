...
For any \TW{} random-oracle-model game with adversary-supplied keys and
adversary $\mathcal{B}$, let $\badleaf^\star$ be the event that two
distinct construction-generated final leaf transcripts receive the same
$\tauleaf$-bit $\RF$ projection. Then
\[
  \Pr[\badleaf^\star]
  \leq
...
\begin{proof}
Build a leaf tag candidate sequence. During
...
leaf tag output-point class $\Ltag[j]$. As the game's construction
computations create distinct final leaf transcripts, append the bridged
input of each immediately before its leaf tag
...
$T$---the second-preimage facet of \Cref{lem:adaptive-candidate} bounds
