\section{Root Tag Hash Security, Committing Security, and Context Discoverability}
\label{sec:cmtds3}
...
that can matter in the hash, committing, and context discoverability games. The
Root Tag Candidate Sequence lemma (\Cref{lem:root-candidate-seq}) collects those inputs
into a chronological candidate sequence, and the Root Tag Hash Bound
(\Cref{lem:root-tag-hash}) turns a collision, fixed-target preimage, or
second-preimage among the candidates into the matching adaptive bound of
\Cref{app:lemmas}, controlling the chance that distinct inputs land on the same
$\tauroot$-bit root tag projection.

The hash random oracle theorems---collision (\Cref{thm:coll-ro}), always
second-preimage (\Cref{thm:asec-ro}), and everywhere preimage
(\Cref{thm:epre-ro})---are then short instantiations, differing only in which
case of the bound they invoke and in whether a leaf tag collision term is needed
when colliding inputs may use different ciphertext bodies. CMTDs-4
(\Cref{thm:cmtds4-ro}) and the remaining committing notions reuse the collision
case. The context discoverability proof of \Cref{thm:cdy-full} uses the same
fixed-target root-preimage case, plus hidden-component guessing.
...
\begin{proof}
The committing and hash (collision, always second-preimage, and everywhere
preimage) games are keyless: all keys are adversary-supplied, so a direct
query to a keyed transcript is one of
$\mathcal{B}$'s visible $\ROflat$ queries counted in $\qRO(\mathcal{B})$,
and there is no $\badkey$ event. The context discoverability proof below
uses the same candidate sequence after separately accounting for the
hidden selector component. No construction oracle runs while
$\mathcal{B}$ issues its direct queries, so each direct-query candidate is
first read no earlier than its append step. The append predicate depends
on $Y$ alone through the exact decoder of \Cref{lem:bridge-decode}, which
ignores the requested output length, and by Output-Point Separation
...
$\qRO(\mathcal{B}) \leq \Nprim$; challenger encryption checks are
...
pairwise $\WiC_3$-distinctness gives pairwise distinct $(K,N,A)$ triples,
...
\subsection{Context Discoverability}
...
valid query-independent selector, so it is at most the success probability
against the valid selector $S^\star$ that samples a fresh context
$(K_0, N_0, A_0)$---each of $K_0$, $N_0$, and $A_0$ uniform on a
$256$-bit space---encrypts a fixed valid message under it to obtain the
valid target ciphertext $C^* = C \concat T^*$, and reveals only the
...
with $w = 256$ and which $\mathcal{A}$ never sees: the only values
revealed to it, the body and tag of $C^*$, are $\RF$ projections
independent of $Z_0$. If $R = R_0$, then $\mathcal{A}$ has reconstructed
the selector's transcript, in particular its $Z_0$-component. A direct
query whose decoded transcript contains a candidate value for $Z_0$ tests
that value against the hidden component, and the final context output is
one additional test. The same union bound as \Cref{lem:key-tests} (hidden
component $Z_0$, $w = 256$) bounds this exact-reconstruction case by
$(\Nprim+1)/2^{256}$.
