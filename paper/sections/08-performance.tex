...
\texttt{amd64} host idle. All reported figures are single-threaded: they
exercise the SIMD axis of \TW{}'s parallelism, and multicore scaling remains
unmeasured. We report a sweep of seven message lengths from $1$\,B
to $16$\,MiB.
...
precomputation and compare steady-state \texttt{Seal}/\texttt{Open} calls under
a fixed key. Each timed \TW{} call still performs the root duplex initialization
inside the call. Both directions are measured for each scheme; the
cycles-per-byte table reports both, and the throughput table reports the
...
$1.81 \to 0.73 \to 0.39$ across the $8$, $32$, and $64$\,KiB columns and is
already at its bulk rate at $64$\,KiB, holding at $0.38$--$0.39$ through
$16$\,MiB. The AVX2 path follows the same shape at lower throughput
($2.21 \to 1.15 \to 1.10$ over the same span, thereafter roughly $1.1$ cycles
per byte); it runs the eight instances as two groups of four rather than in a
single $8$-wide register, leaving the AVX-512 core about $2.9\times$ faster in
bulk. The \texttt{arm64} curve flattens earliest
($1.97 \to 0.92 \to 0.89 \to 0.89 \to 0.91$ cycles per byte from $8$\,KiB to
$16$\,MiB): the eight-way core itself processes its batch as four two-instance
pairs, so the fused $2$-wide kernel runs at essentially the same per-chunk rate,
and a $32$\,KiB message---chunk~$0$ and three leaves as two full pairs---is
already close to the long-message rate. In wall-clock terms \TW{} reaches
...
this out: per-message latency is nearly flat across them at roughly $571$
reference cycles on \texttt{amd64} ($571$ at $1$\,B versus $9.04 \times 64
\approx 578$ at $64$\,B) and $620$ core cycles on \texttt{arm64} ($620$ versus
$9.88 \times 64 \approx 633$), i.e.\ about $285$ and $310$ cycles per
...
value relative to AES-128-GCM lies in the tree structure and committing security
properties analyzed in \Cref{sec:aead-security,sec:root-tag-hash-security}
