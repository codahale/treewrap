...
whose real-vs-ideal form adds the ideal-side proximity term
$\Lift_c(\Nprim)$, and the all-in-one AE bound is the single-user corollary. The section then
...
\[
  ...
  + \frac{\qv^{(d)}}{2^{\tauroot}}.
\]
In outline, \Cref{lem:mu-adjacent-hybrid} proceeds through the explicit
game sequence of \Cref{def:adjacent-hybrid-games}: an instrumented
implementation of $H_{d-1}^\star$ and an all-reject game share all code
except the return statement of a successful non-replay user-$d$
verification comparison, which sets the flag $\forge$, while a direct
$\ROflat$ query decoding to the hidden key $K_d$ sets the flag
$\badkey$. The real pair agrees pointwise until $\forge$. Claim~3 in
the proof of \Cref{lem:mu-adjacent-hybrid} couples the all-reject game
to $H_d^\star$ until $\badkey$: Encryption Transcript Marginal
Uniformity (\Cref{lem:enc-transcript-marginal-uniformity}) supplies
freshness and distinctness for newly generated user-$d$ encryption
points, while the all-reject structure ensures that any reused
user-$d$ root-tag entry was previously read only through constant
reject answers and remains uniform conditioned on the visible
transcript. Environment lookups for the other users decode to their own
keys and touch neither flag (\Cref{lem:env-neutrality}). A
first-occurrence
decomposition of $\forge \vee \badkey$ in the all-reject game then
yields the three terms: $\badkey$ is charged at most
$\KeyGuess_k(\qRO(\mathcal{B}))$ by the foreign-key key-test claim of
\Cref{app:mu-verification-coupling}; a same-slot collision between two
user-$d$ final leaf tags---at least one reached by an encryption
computation---is charged at most $\nleaf^{(d)}/2^{\tauleaf}$ by the
same-slot case of \Cref{lem:leaf-collision}; and a successful
comparison before both events must lie in a verification-first
transcript class and is charged at most $\qv^{(d)}/2^{\tauroot}$ by
\Cref{lem:ver-first-root-guess}.
...
the corresponding $\Ltag(j)$ class. Encryption queries are nonce-respecting.
Couple the real \textsf{INT-RUP} execution to the all-reject continuation
used in the proof of \Cref{lem:int-rup-root-guess}. Up to the first
accepting non-replay submission, the construction computations and
leaf-tag table accesses agree; the continuation differs only by
deferring the relevant final root-tag value until comparison. Thus the
internal event $\badleaf$ transfers to the all-reject continuation, where
the same-slot case of \Cref{lem:leaf-collision}, applied to the enlarged
