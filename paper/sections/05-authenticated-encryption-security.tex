...
$\badkey$. The real pair agrees pointwise until $\forge$; the
all-reject game agrees in distribution with $H_d^\star$ until
$\badkey$, since by Encryption Transcript Marginal Uniformity
(\Cref{lem:enc-transcript-marginal-uniformity}) each user-$d$
encryption answer is fresh uniform given the visible transcript; and
environment lookups for the other users decode to their own keys and
touch neither flag (\Cref{lem:env-neutrality}). A first-occurrence
