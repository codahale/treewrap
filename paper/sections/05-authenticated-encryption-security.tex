...
ones. Nonce-respecting encryption and Transcript Injectivity
(\Cref{lem:transcript-inj}) place every real user-$d$ encryption output
at fresh $\ROflat$ points, so the coupled ideal coordinate has the true
...
the adaptive key-test bound (\Cref{lem:key-tests}); two user-$d$ final
leaf transcripts sharing a key, nonce, and leaf index collide on their
...
nonce-respecting, plaintext-computing replies are keystream projections
at stream output points, and the visible view agrees with the
