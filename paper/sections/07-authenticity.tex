...
$(N, A, M)$ returns $(C, T) = \Enc_K^\RO(N, A, M)$ and records the
...
\[
  ...
  + \Pr[\badleaf \text{ before }
        \badkey].
\]
\Cref{lem:int-leaf-fresh-before-badkey} supplies the event premise for
the Leaf-Tag Collision Bound (\Cref{lem:leaf-collision}): before
$\badkey$, no direct query has pre-read the
bridged input of a hidden-key final leaf transcript before the
construction first creates that transcript. Thus
$\{\badleaf \text{ before }
...
encryption-first, \Cref{lem:int-encryption-first-replay} says the
submission cannot be an accepting non-replay; the remaining accepting
non-replay submissions are therefore verification-first. By
...
follows the SpongeWrap authenticity result of \cite[Lemma~6]{BDPV11},
whose random-oracle bound carries no sponge-capacity term. We realize
