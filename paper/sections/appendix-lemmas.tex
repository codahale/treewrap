...
bound. The remaining interfaces expose the local events consumed by the
main proofs: encryption-side freshness, verification hidden-key exposure
and root tag guessing, leaf tag collisions, and final-root candidate
sequencing. The tag hash proof applies the candidate bound, committing
statements reuse the resulting root-collision bound, and the mu-ind and
\textsf{INT-RUP} proofs derive the premises for their accounting lemmas.
...
(\Cref{lem:root-candidate-seq,lem:root-tag-hash}) of \Cref{sec:root-tag-hash-security},
the tag hash proof; the committing consequences reuse the same root-collision
bound under their opening constraints. The mu-ind proof reuses the same
structural lemmas through the explicit hybrid game sequence of
\Cref{def:adjacent-hybrid-games}, adding freshness, key-test, leaf-collision,
and verification-first root guessing accounting.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[consumer] (c8) at (15.2, -1.3) {Tag hash proofs;\\CMT corollaries};
    ...
    \begin{scope}[on background layer]
      ...
      % Tag hash consumers and committing consequences.
...
\begin{align*}
  \Rtag   &= \text{root transcript prefix whose final call is a \sigagglast{} aggregation call}, \\
  \Ltag(j)   &= \text{leaf-$j$ transcript prefix whose final call is a \sigmsglast{} message call}.
\end{align*}
...
Intermediate construction transcript lookup points (initialization, AD,
non-final message, non-final aggregation, and the final root message
call that every $n \geq 1$ computation reaches with zero requested
output) are also typed prefixes covered by \Cref{lem:bridge-decode};
the exact keyed direct-query decoder names them in
\Cref{tab:keyed-twout}.
...
initialization, AD blocks, and root ciphertext blocks, and, when
$n \geq 1$, the same aggregation input and leaf tag sequence.
\end{lemma}
...
is reached by an encryption computation, it suffices that no
construction-generated final leaf transcript receives the same leaf tag
as a distinct encryption-reached final leaf transcript with the same
...
\Cref{sec:root-tag-hash-security,sec:aead-security} and \Cref{cor:committing-combined} are
...
(\Cref{sec:resources}) but decode to $\bot$ under the keyed
direct-query decoder of \Cref{def:keyed-twout}, so they cannot trigger
the key-hit abort events that later game analyses define from its
counted classes.
\end{proof}

The bridge injectivity established here underlies Opening Triple
Separation (\Cref{cor:triple-sep}, \Cref{sec:root-tag-hash-security}): distinct
...
\item \Ragg (root duplex, suffix \sigaggmore),
\item $\Rtag$ (root duplex, suffix \sigagglast).
\end{itemize}
For any two such prefixes whose bridged flattened inputs are equal,
bridge decoding (\Cref{lem:bridge-decode}) followed by the separation of
\Cref{lem:transcript-inj} recovers the same duplex
...
$\bits^k \times \mathcal{C}_{\mathsf{out}}$, where $\mathcal{C}_{\mathsf{out}}$
is the finite set of output-point classes of \Cref{tab:keyed-twout}. The map ignores
...
key projection $(K,\mathcal{C})$ throughout.
\end{definition}
...
\label{lem:key-tests}
Consider a \TW{} random oracle model game with a hidden key $K$ sampled
uniformly from $\bits^k$, direct adversary access to
$\ROflat$, and an immediate abort event $\badkey$ that fires on a
direct query $(Y,\ell)$ exactly when $Y$ decodes under $\KeyedTWOut$ to
the candidate key $K$ and a counted
output-point class of \Cref{tab:keyed-twout}. Suppose that, before the
...
queries, the hidden $K$ is uniform on $\bits^k \setminus S_i$. Then
\[
  \Pr[\badkey]
  \leq \KeyGuess_k(\qRO)
  = \qRO / 2^k,
...
\begin{proof}
If $\qRO \geq 2^k$ the right-hand side $\qRO/2^k$ is at least one, so the
claim is trivial. Assume $\qRO < 2^k$.
...
queries whose decoded candidate key already lies in the current set
$S_i$, do
...
For a fixed position $i \in \{1, \dots, \qRO\}$, let $E_i$ be the
event that the $i$-th fresh key test occurs and its candidate key
$K'_i$ equals the hidden $K$. Because $\badkey$ aborts the game
immediately, $E_i$ implies that the first $i-1$ fresh tests all
occurred and all missed; if some earlier fresh test never occurs then
$E_i$ is false and the bound below still holds, as in the proof of
\Cref{lem:adaptive-candidate}.

Fix $j \in \{0, \dots, i-2\}$ and condition on any positive-probability
visible prefix before the $(j+1)$-st fresh test on which the first $j$
fresh tests occurred and missed. The invariant says that the hidden $K$
is then uniform over the $2^k - j$ values not yet tested, and the next
fresh candidate key $K'_{j+1}$ is one of those remaining values and is
determined by the adversary's next direct query, so the $(j+1)$-st
fresh test misses with probability $1 - 1/(2^k - j)$. Chaining these
conditional miss probabilities over the fixed steps
$j = 0, \dots, i-2$ telescopes to
\[
  \Pr[\text{the first $i-1$ fresh tests occur and miss}]
  \leq \prod_{j=0}^{i-2} \left(1 - \frac{1}{2^k - j}\right)
  = \prod_{j=0}^{i-2} \frac{2^k - j - 1}{2^k - j}
  = \frac{2^k - i + 1}{2^k}.
\]
Conditioned on any such prefix, the $i$-th fresh test hits the hidden
$K$ with probability $1/(2^k - i + 1)$, so
\[
  \Pr[E_i]
  \leq \frac{2^k - i + 1}{2^k} \cdot \frac{1}{2^k - i + 1}
  = \frac{1}{2^k}.
\]
The first $\badkey$ abort occurs at a fresh key test, because
$\KeyedTWOut$ returns either $\bot$ or a unique candidate key and a
repeat of a previously tested candidate would already have fired the
abort. Hence $\badkey \subseteq \bigcup_{i=1}^{\qRO} E_i$, and a union
bound over the $\qRO$ positions gives
$\Pr[\badkey] \leq \qRO / 2^k$.
\end{proof}
...
reject. Let
$\badleaf^\star$ be the event that two distinct construction-generated final
...
\item \emph{Hidden key, same slot.} Suppose the game has a hidden key,
  ...
  final leaf transcript that shares its slot and is reached by an
...
$\badleaf^\star$ pair is a pair of construction-generated candidates.
...
construction-generated hidden-key transcripts sharing a slot, with at
least one member reached by an encryption computation. Nonce-respecting
...
transcripts, so each slot contains at most one encryption-reached final
leaf transcript. That transcript may have first been created by an
...
Charge each possible $\badleaf$ pair to its member that is not reached by
an encryption computation. Each charged transcript has one slot, and
that slot has at most one encryption-reached counterpart, so each charged
transcript receives at most one charge. Thus at most
$\nleaf(\mathcal{B})$ charged pairs can occur.
...
construction computation first creates, or an encryption computation
first reaches, a final leaf transcript whose
...
the two reached by an encryption computation---the same-slot event of
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{Enc}{$U$, $A$, $M$}
      ...
      \State set $\badleaf$ if a final leaf transcript first created or
        first reached here completes a same-slot collision
    ...
    \Procedure{Vf}{$U$, $A$, $C \concat T$}
      ...
      \State determine the verification transcript of $(K_d, U, A, C)$;
        ...
        leaf transcripts, setting $\badleaf$ when a transcript first
        created here completes a same-slot collision, and nothing at its
        stream output points
...
first occurrence of $\badkey \vee \badleaf$---with $\badleaf$ the
...
Let $\mathcal{B}$ be a single-key \textsf{INT-RUP} adversary in the
\TW{} random oracle model, with nonce-respecting encryption queries and
the resources of \Cref{sec:resources}. In its
encryption/plaintext-computing/verification interaction with hidden key
$K$, let $\badkey$ be the event that some direct $\ROflat$ query input
$Y$ satisfies $\KeyedTWOut(Y) = (K,\mathcal{C})$ for a counted class
$\mathcal{C}$ of \Cref{tab:keyed-twout}, and let $\badleaf$ be the
same-slot event of \Cref{lem:leaf-collision}, part~2, over the
construction computations generated by encryption, plaintext-computing,
and verification. Let $\forge$ be the event that some valid non-replay
verification query accepts. Then
\[
...
The multi-user indistinguishability theorem (\Cref{thm:mu-ind-ro}) telescopes a
