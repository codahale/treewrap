...
all miss the hidden $K$. The invariant says that, conditioned on the
visible prefix before the $(i+1)$-st fresh test and on $F_i$, the
hidden $K$ is uniform over the $2^k - i$ values not yet tested. The next
fresh candidate key $K'_{i+1}$ is one of those remaining values and is
...
hidden $K$ is $m / 2^k \leq \qRO / 2^k$. This event is exactly the
first $\badkey$ abort, because
$\KeyedTWOut$ returns either $\bot$ or a unique candidate key.
\end{proof}
