...
sequence; the second is a same-slot freshness argument in the
...
\item \emph{Hidden key, same slot.} Suppose the game has a hidden key
  and nonce-respecting encryption queries: work in the all-reject
  continuation of the interaction, in which every non-replay
  verification submission performs its construction computation but
  returns reject. Say two final
...
In the stopped single-key \TW{} random oracle encryption/verification
interaction before $\badkey \cup \badleaf$---with $\badleaf$ the
same-slot event of \Cref{lem:leaf-collision}, part~2---let
...
The same argument excludes accepting submissions in encryption-first
classes: such a class is first generated by an encryption computation,
so by the final-root determinacy of \Cref{lem:transcript-inj} and
Leaf-Transcript Recovery (\Cref{lem:leaf-recovery})---whose hypothesis
$\neg\badleaf$ supplies because the comparand is that encryption
computation---any non-replay submission in the class carries the same
$(U,A,C)$ and either replays the recorded tuple or fails the tag
comparison. On $\mathsf{win}_{\mathsf{vf}}$ the first accepting
non-replay submission is therefore itself verification-first.
...
with the same sampled value. Before $\badkey^{(d)}$ the lemma's
hypothesis $\neg\mathsf{hit}_{\mathsf{fresh}}$ holds for every user-$d$
encryption query: a direct query pre-reading the bridged input of a
user-$d$ hidden-key counted point would decode under $\KeyedTWOut$ to
$K_d$ and a counted class (\Cref{lem:key-renaming}) and would itself be
$\badkey^{(d)}$. The present environment adds a real
