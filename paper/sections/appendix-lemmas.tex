...
\paragraph{Bounding the three terms.} For $\Pr[\badkey^{(d)}]$, adapt the
single-key visible-prefix renaming coupling of
\Cref{lem:ver-key-uniformity} to the environment $\mathcal{E}_d$. Its
conclusion does not transfer verbatim: the foreign users $e > d$ share the
lazy $\ROflat$ table, so renaming $K_d$ onto a foreign key would alter
their visible transcripts, and the renaming orbit must exclude the foreign
keys rather than range over all of $\bits^k \setminus S_i$. What the
coupling still delivers in $\mathcal{E}_d$ is the marginal per-candidate
bound, which is all the key test needs. The key-test contribution is
...
$\qRO(\mathcal{B})$ such tests, the union bound of \Cref{lem:key-tests}
packages these into
