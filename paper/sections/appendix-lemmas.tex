...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \begin{scope}[on background layer]
      ...
      % privacy and INT-CTXT accounting.
      \draw[arr] (ti) -- (kr);
  ...
  \caption{Dependency graph of the supporting lemmas in this
    ...
    renaming used by the privacy and INT-CTXT accounting. The Adaptive
    ...
    INT-CTXT through its hidden-key bound and the committing collision and
    second-preimage proofs through its adversary-supplied-key bound. The
...
The privacy and INT-CTXT accounting both reindex hidden-key transcript
addresses by the key field. We record that reindexing once.
...
$\mathcal{V}$ under this conditioning. For every $K' \in \mathcal{V}$, no
prior direct query lies in the hidden-key address set $\mathcal{Y}_{K'}$
of \Cref{lem:key-renaming} (direct-query avoidance, since $K' \notin S$),
so running $\Enc_{K'}^\RO$ on the pending query reaches no counted
transcript point queried directly before; and the address sets
$\mathcal{Y}_{K'}$ for distinct $K' \in \mathcal{V}$ are pairwise
disjoint (renaming-lemma disjointness).
