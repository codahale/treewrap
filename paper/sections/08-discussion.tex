...
context discoverability security (CDY).
...
listed schemes do not provide it. The CDY column records context discoverability
security in the \cite{MLGR23} sense: \TW{} provides it within the mode
...
notion \cite{MLGR23}, so its absence of entries reflects the state of the
literature rather than a positive or negative result.
...
AES-GCM \cite{Dwo07}         & AES                & yes & $16/12/16$ & yes & no  & no  & no  \\
...
bounds of \Cref{sec:cmtds3} do not rest on soundness of a tree coding at all:
...
enters only the collision bound of \Cref{thm:coll}, and there only
...
the strongest full-input committing notion (\textsf{CMT-4})---as a proven,
...
additionally yields context discoverability security against that fixed tag, which the
...
conservative in a way that points to an open problem. The flat sponge
...
$\Lift_c$ already meets the $128$-bit target (\Cref{sec:bound-discussion}).
