...
together in vector registers, allowing one vector operation to update the
same lane across several duplex states.

\paragraph{Chunk~$0$ fusion and tail handling.}
...
\texttt{arm64} batched kernels process the complete chunk and store the
closing states back in assembly, which is the writeback needed for chunk~$0$
fusion.
